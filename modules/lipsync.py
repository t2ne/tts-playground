...
# Add Wav2Lip to path
WAV2LIP_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'Wav2Lip')
...
def generate_lipsync_wav2lip(audio_file, face_file=AVATAR_FACE, filename="output.mp4"):
    ...
    if not os.path.exists(checkpoint_path):
        print(f"⚠️  Wav2Lip checkpoint not found at {checkpoint_path}")
    ...
    try:
        ...
        if result.returncode == 0 and os.path.exists(output_path):
            ...
            print(f"⚠️  Wav2Lip failed: {result.stderr}")
            print("🔄 Falling back to basic video generation...")
            return generate_lipsync_basic(audio_file, face_file, filename)
